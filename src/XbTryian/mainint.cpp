...
static bool helpSystemPage(Uint8* topic, bool* restart)
{
	...
	for (; ; )
	{
		...
		wsprintfA(buffer, "%s %d", miscText[24], page - topicStart[*topic - 1] + 1);
		draw_font_hv(VGAScreen, 10, 192, buffer, small_font, left_aligned, 13, 5);

		wsprintfA(buffer, "%s %d of %d", miscText[25], page, MAX_PAGE);
	...
	for (; ; )
	{
		...
		for (size_t i = 0; i < menuItemsCount; ++i)
		{
			...
			if (disabled)
			{
				...
				wsprintfA(buffer, "%s -----", miscTextB[2]);
			...
			else
			{
				...
				wsprintfA(buffer, "%s %s", miscTextB[2], saveFile->levelName);
				JE_textShade(VGAScreen, xMenuItemLastLevel, y, buffer, 5, selected ? 6 : 2, FULL_SHADE);

				wsprintfA(buffer, "%s %u", miscTextB[1], saveFile->episode);
...
void JE_highScoreScreen(void)
{
	...
	for (; ; )
	{
		...
		for (Uint8 i = 0; i < 3; ++i)
		{
			...
			wsprintfA(buffer, "~#%d:~  %d", i + 1, saveFile->highScore1);
		...
		if (slot < slot_limit)
		{
...

			{
				...
				do
				{
					...
					if (twoPlayerMode)
					{
						wsprintfA(buffer, "%s %s", miscText[48 + p], miscText[53]);
					...
					wsprintfA(buffer, "%s %d", miscText[37], temp_score);
				...
				for (int i = first_slot; i < slot_limit; ++i)
				{
					if (i != slot)
					{
						wsprintfA(buffer, "~#%d:~  %d", (i - first_slot + 1), saveFiles[i].highScore1);
				...
				wsprintfA(buffer, "~#%d:~  %d", (slot - first_slot + 1), saveFiles[slot].highScore1);
...
bool load_next_demo(void)
{
	...
	wsprintfA(demo_filename, "demo.%d", demo_num);
	...
	do { char _dm[256]; wsprintfA(_dm, "loaded demo '%s'\n", demo_filename); OutputDebugStringA(_dm); } while (0);
...
void JE_endLevelAni(void)
{
	...
	{
		wsprintfA(tempStr, "%s %s", miscText[27 - 1], levelName); // "Completed"
		JE_outTextGlow(VGAScreenSeg, 20, 20, tempStr);
	}
	else
	{
		wsprintfA(tempStr, "%s %s", miscText[62 - 1], levelName); // "Exiting"
		JE_outTextGlow(VGAScreenSeg, 20, 20, tempStr);
	}
	...
	if (twoPlayerMode)
	{
		for (uint i = 0; i < 2; ++i)
		{
			wsprintfA(tempStr, "%s %lu", miscText[40 + i], player[i].cash);
			JE_outTextGlow(VGAScreenSeg, 30, 50 + 20 * i, tempStr);
		}
	}
	else
	{
		wsprintfA(tempStr, "%s %lu", miscText[28 - 1], player[0].cash);
		JE_outTextGlow(VGAScreenSeg, 30, 50, tempStr);
	}

	temp = (totalEnemy == 0) ? 0 : ((enemyKilled * 100) / totalEnemy);
	wsprintfA(tempStr, "%s %d%%", miscText[63 - 1], temp);
...
void JE_inGameDisplays(void)
{
	...
	{
		wsprintfA(tempstr, "%lu", player[i].cash);
	...
	if (onePlayerAction || twoPlayerMode)
	{
		for (int temp = 0; temp < (onePlayerAction ? 1 : 2); temp++)
		{
			...
			if (extra_lives >= 5)
			{
				...
				tempW = (temp == 0) ? 45 : 250;
				wsprintfA(tempstr, "%d", extra_lives);
...
redo:
	...
	if (this_player->is_alive && !endLevel)
	{
		...
		else  /*twoPlayerLinked*/
		{
			...
			b = player_shot_create(0, SHOT_MISC, this_player->x + 1 + mainint_ftoi(sinf(linkGunDirec + 0.2f) * 26), this_player->y + mainint_ftoi(cosf(linkGunDirec + 0.2f) * 26), *mouseX_, *mouseY_, 148, playerNum_);
			shotMultiPos[SHOT_MISC] = 0;
			b = player_shot_create(0, SHOT_MISC, this_player->x + 1 + mainint_ftoi(sinf(linkGunDirec - 0.2f) * 26), this_player->y + mainint_ftoi(cosf(linkGunDirec - 0.2f) * 26), *mouseX_, *mouseY_, 148, playerNum_);
			shotMultiPos[SHOT_MISC] = 0;
			b = player_shot_create(0, SHOT_MISC, this_player->x + 1 + mainint_ftoi(sinf(linkGunDirec) * 26), this_player->y + mainint_ftoi(cosf(linkGunDirec) * 26), *mouseX_, *mouseY_, 147, playerNum_);
			...
			else if (button[1 - 1])
			{
				...
				b = player_shot_create(0, SHOT_REAR, this_player->x + 1 + mainint_ftoi(sinf(linkGunDirec) * 20), this_player->y + mainint_ftoi(cosf(linkGunDirec) * 20), *mouseX_, *mouseY_, linkGunWeapons[this_player->items.weapon[REAR_WEAPON].id - 1], playerNum_);
		...
		if (this_player->is_alive)
		{
			if (!endLevel)
			{
				...
				case 2:  // front-mounted
					...
					break;
				case 4:  // orbiting
					this_player->sidekick[LEFT_SIDEKICK].x = this_player->x + mainint_ftoi(sinf(optionSatelliteRotate) * 20);
					this_player->sidekick[LEFT_SIDEKICK].y = this_player->y + mainint_ftoi(cosf(optionSatelliteRotate) * 20);
					break;
				}
				...
				{
				case 4:  // orbiting
					this_player->sidekick[RIGHT_SIDEKICK].x = this_player->x - mainint_ftoi(sinf(optionSatelliteRotate) * 20);
					this_player->sidekick[RIGHT_SIDEKICK].y = this_player->y - mainint_ftoi(cosf(optionSatelliteRotate) * 20);
					break;
				case 1:  // trailing
				case 3:
		...
		if (enemyAvail[z] != 1)
		{
			...
			{   /*Collide*/
				...
				if (evalue > 29999)
				{
					...
					else if (evalue > 32100)
					{
						if (playerNum_ == 1)
						{
							...
							if (isNetworkGame)
								wsprintfA(tempStr, "%s %s %s", JE_getName(1), miscTextB[4 - 1], special[evalue - 32100].name);
							else if (twoPlayerMode)
								wsprintfA(tempStr, "%s %s", miscText[43 - 1], special[evalue - 32100].name);
							else
								wsprintfA(tempStr, "%s %s", miscText[64 - 1], special[evalue - 32100].name);
					...
					else if (evalue > 32000)
					{
						if (playerNum_ == 2)
						{
							enemyAvail[z] = 1;
							if (isNetworkGame)
								wsprintfA(tempStr, "%s %s %s", JE_getName(2), miscTextB[4 - 1], options[evalue - 32000].name);
							else
								wsprintfA(tempStr, "%s %s", miscText[44 - 1], options[evalue - 32000].name);
							JE_drawTextWindow(tempStr);
						...
						{
							enemyAvail[z] = 1;
							wsprintfA(tempStr, "%s %s", miscText[64 - 1], options[evalue - 32000].name);
							JE_drawTextWindow(tempStr);
					...
					else if (evalue > 31000)
					{
						...
						{
							if (isNetworkGame)
								wsprintfA(tempStr, "%s %s %s", JE_getName(2), miscTextB[4 - 1], weaponPort[evalue - 31000].name);
							else
								wsprintfA(tempStr, "%s %s", miscText[44 - 1], weaponPort[evalue - 31000].name);
							JE_drawTextWindow(tempStr);
						...
						{
							wsprintfA(tempStr, "%s %s", miscText[64 - 1], weaponPort[evalue - 31000].name);
							JE_drawTextWindow(tempStr);
					...
					else if (evalue > 30000)
					{
						...
						{
							if (isNetworkGame)
								wsprintfA(tempStr, "%s %s %s", JE_getName(1), miscTextB[4 - 1], weaponPort[evalue - 30000].name);
							else
								wsprintfA(tempStr, "%s %s", miscText[43 - 1], weaponPort[evalue - 30000].name);
							JE_drawTextWindow(tempStr);
						...
						{
							wsprintfA(tempStr, "%s %s", miscText[64 - 1], weaponPort[evalue - 30000].name);
							JE_drawTextWindow(tempStr);
							player[0].items.weapon[FRONT_WEAPON].id = evalue - 30000;
							shotMultiPos[SHOT_FRONT] = 0;
							enemyAvail[z] = 1;
				...
				else if (enemy[z].scoreitem)
				{
					...
					{
						if (isNetworkGame)
							wsprintfA(tempStr, "%s %s %s", JE_getName(1), miscTextB[4 - 1], miscText[45 - 1]);
						else if (twoPlayerMode)
							wsprintfA(tempStr, "%s %s", miscText[43 - 1], miscText[45 - 1]);
						else
					...
					{
						if (isNetworkGame)
							wsprintfA(tempStr, "%s %s %s", JE_getName(2), miscTextB[4 - 1], miscText[46 - 1]);
						else if (twoPlayerMode)
							wsprintfA(tempStr, "%s %s", miscText[44 - 1], miscText[46 - 1]);
						else
