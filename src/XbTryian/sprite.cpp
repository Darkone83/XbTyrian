...
void load_sprites(unsigned int table, FILE* f)
{
	...
	if (!(sprite_table[table].count <= SPRITES_PER_TABLE_MAX)) for (;;) Sleep(1000);
...
void blit_sprite(SDL_Surface* surface, int x, int y, unsigned int table, unsigned int index)
{
	...
	{
		OutputDebugStringA("sprite assert failed\n");
		return;
	}
	...
	if (!(surface->format->BitsPerPixel == 8)) for (;;) Sleep(1000);
...
void JE_loadCompShapes(Sprite2_array* sprite2s, char s)
{
	...
	char buffer[20];
	wsprintfA(buffer, "newsh%c.shp", tolower((unsigned char)s));
...
{
	if (!(sprite2s->data == NULL)) for (;;) Sleep(1000);
...
void JE_loadMainShapeTables(const char* shpfile)
{
	...
	if (!(shpNumb + 1u == COUNTOF(shpPos))) for (;;) Sleep(1000);
