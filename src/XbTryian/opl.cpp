...
#define fltype double
  ...
  // Xbox float→int helpers — avoids __ftol2_sse missing from Xbox CRT.
  // Placed after #define fltype so all types are in scope.
static __inline long opl_ftol(double x)
{
	long result;
	__asm fld x
	__asm fistp result
...
static __inline unsigned long opl_ftoul(double x)
{
...
void operator_output(op_type* op_pt, Bit32s modulator, Bit32s trem) {
	if (op_pt->op_state != OF_TYPE_OFF) {
		...
		op_pt->cval = opl_ftol(op_pt->step_amp * op_pt->vol * op_pt->cur_wform[i & op_pt->cur_wmask] * trem / 16.0);
	}
}
...
void change_feedback(Bitu chanbase, op_type * op_pt) {
	...
	if (feedback) op_pt->mfbi = opl_ftol(pow(FL2, (fltype)((feedback >> 1) + 8)));
	else op_pt->mfbi = 0;
}
...
void change_frequency(Bitu chanbase, Bitu regbase, op_type * op_pt) {
	...
	// 20+a0+b0:
	op_pt->tinc = opl_ftoul((((fltype)(frn << oct)) * frqmul[adlibreg[ARC_TVS_KSR_MUL + regbase] & 15]));
	// 40+a0+b0:
...
void adlib_init(Bit32u samplerate) {
	...
	generator_add = opl_ftoul(INTFREQU * FIXEDPT / int_samplerate);
	...
	vibtab_add = opl_ftoul(VIBTAB_SIZE * FIXEDPT_LFO / 8192 * INTFREQU / int_samplerate);
	vibtab_pos = 0;
	...
	for (i = 0; i < TREMTAB_SIZE; i++) {
		...
		trem_table[i] = opl_ftol(pow(FL2, trem_val1) * FIXEDPT);
		trem_table[TREMTAB_SIZE + i] = opl_ftol(pow(FL2, trem_val2) * FIXEDPT);
	}

	// tremolo at 3.7hz
	tremtab_add = opl_ftoul((fltype)TREMTAB_SIZE * TREM_FREQ * FIXEDPT_LFO / (fltype)int_samplerate);
	tremtab_pos = 0;
	...
	if (!initfirstime) {
		...
		for (i = 0; i < (WAVEPREC >> 1); i++) {
			wavtable[(i << 1) + WAVEPREC] = (Bit16s)opl_ftol(16384 * sin((fltype)((i << 1)) * PI * 2 / WAVEPREC));
			wavtable[(i << 1) + 1 + WAVEPREC] = (Bit16s)opl_ftol(16384 * sin((fltype)((i << 1) + 1) * PI * 2 / WAVEPREC));
			wavtable[i] = wavtable[(i << 1) + WAVEPREC];
			// alternative: (zero-less)
/*			wavtable[(i<<1)  +WAVEPREC]	= (Bit16s)opl_ftol(16384*sin((fltype)((i<<2)+1)*PI/WAVEPREC));
			wavtable[(i<<1)+1+WAVEPREC]	= (Bit16s)opl_ftol(16384*sin((fltype)((i<<2)+3)*PI/WAVEPREC));
		...
		if (adlibreg[ARC_PERC_MODE] & 0x20) {
			...
			if (adlibreg[ARC_FEEDBACK + 6] & 1) {
				...
				if (cptr[9].op_state != OF_TYPE_OFF) {
					if (cptr[9].vibrato) {
						vibval1 = vibval_var1;
						for (i = 0; i < endsamples; i++)
							vibval1[i] = opl_ftol((vib_lut[i] * cptr[9].freq_high / 8) * FIXEDPT * VIBFAC);
					}
				...
				if ((cptr[9].op_state != OF_TYPE_OFF) || (cptr[0].op_state != OF_TYPE_OFF)) {
					if ((cptr[0].vibrato) && (cptr[0].op_state != OF_TYPE_OFF)) {
						vibval1 = vibval_var1;
						for (i = 0; i < endsamples; i++)
							vibval1[i] = opl_ftol((vib_lut[i] * cptr[0].freq_high / 8) * FIXEDPT * VIBFAC);
					}
					else vibval1 = vibval_const;
					if ((cptr[9].vibrato) && (cptr[9].op_state != OF_TYPE_OFF)) {
						vibval2 = vibval_var2;
						for (i = 0; i < endsamples; i++)
							vibval2[i] = opl_ftol((vib_lut[i] * cptr[9].freq_high / 8) * FIXEDPT * VIBFAC);
					}
			...
			if (op[8].op_state != OF_TYPE_OFF) {
				...
				if (cptr[0].vibrato) {
					vibval3 = vibval_var1;
					for (i = 0; i < endsamples; i++)
						vibval3[i] = opl_ftol((vib_lut[i] * cptr[0].freq_high / 8) * FIXEDPT * VIBFAC);
				}
			...
			if ((op[7].op_state != OF_TYPE_OFF) || (op[16].op_state != OF_TYPE_OFF) ||
				...
				if ((cptr[9].vibrato) && (cptr[9].op_state == OF_TYPE_OFF)) {
					vibval4 = vibval_var2;
					for (i = 0; i < endsamples; i++)
						vibval4[i] = opl_ftol((vib_lut[i] * cptr[9].freq_high / 8) * FIXEDPT * VIBFAC);
				}
