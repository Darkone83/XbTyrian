...
#include <stdint.h>
 ...
 // Local float→int helper to avoid __ftol2_sse
static __inline int tyrian2_ftoi(float x)
{
	int result;
	__asm fld x
	__asm fistp result
...
/*
 * XDK / single-player port compatibility globals.
 *
 * These are referenced by the old OpenTyrian gameplay modules through
 * network/menu glue headers, but this Xbox project is not linking the
 * original translation unit that normally owns them.  Define them once
 * here so mainint.obj and tyrian2.obj resolve cleanly.
 */
bool yourInGameMenuRequest = false;
bool nortShipRequest = false;
bool haltGame = false;
bool helpRequest = false;
bool moveOk = true;
bool skipLevelRequest = false;
bool pauseRequest = false;
bool inGameMenuRequest = false;

unsigned int thisPlayerNum = 1;

static char network_player_name_storage[32] = "";
static char network_opponent_name_storage[32] = "";
char* network_player_name = network_player_name_storage;
char* network_opponent_name = network_opponent_name_storage;

#ifdef _XBOX
static void XbTryian_ReturnToDashboard(void)
{
...
inline static void blit_enemy(SDL_Surface* surface, unsigned int i, signed int x_offset, signed int y_offset, signed int sprite_offset)
{
	...
	{
		do { char _m[80]; wsprintfA(_m, "warning: enemy %d sprite missing\n", i); OutputDebugStringA(_m); } while (0);
		return;
	}
						...
						default:
							...
							for (int tempCount = weapons[temp3].multi; tempCount > 0; tempCount--)
							{
								...
								if (weapons[temp3].aim > 0)
								{
									...
									enemyShot[b].sxm = tyrian2_ftoi((float)relative_x / longest_side * aim);
									enemyShot[b].sym = tyrian2_ftoi((float)relative_y / longest_side * aim);
								}
							}
							break;
						}
					...
					if (b > 0)
					{
						...
						if (e->launchtype > 0 && e->launchfreq == 0)
						{
							...
							else
							{
								...
								e->exc = tyrian2_ftoi(((float)target_x / longest_side) * e->launchtype);
								e->eyc = tyrian2_ftoi(((float)tempI5 / longest_side) * e->launchtype);
							}
						}
...
start_level_first:
	...
	if (!(COUNTOF(player->old_x) == COUNTOF(player->old_y))) for (;;) Sleep(1000);
	...
	if (!play_demo && record_demo)
	{
		...
		do
		{
			wsprintfA(tempStr, "demorec.%d", new_demo_num++);
...
level_loop:
	...
	/*---------------------------EVENTS-------------------------*/
	while (eventLoc <= maxEvent && eventRec[eventLoc - 1].eventtime <= curLoc)
		JE_eventSystem();
							...
							for (temp2 = 0; temp2 < 100; temp2++)
							{
								if (enemyAvail[temp2] != 1)
								{
											...
											((temp3 > 40) && (temp3 / 20 == temp / 20) && (temp3 <= temp)))))
									{
										...
										{
											if (!((unsigned int)enemy[temp2].flagnum - 1 < COUNTOF(globalFlags))) for (;;) Sleep(1000);
	...
	if (debug)
	{
		...
		tempStr[9] = '\0';
		wsprintfA(buffer, "SM = %s", tempStr);
		JE_outText(VGAScreen, 30, 70, buffer, 4, 0);

		wsprintfA(buffer, "Memory left = %d", -1);
		JE_outText(VGAScreen, 30, 80, buffer, 4, 0);
		wsprintfA(buffer, "Enemies onscreen = %d", enemyOnScreen);
		JE_outText(VGAScreen, 30, 90, buffer, 6, 0);

		debugHist = debugHist + abs((JE_longint)debugTime - (JE_longint)lastDebugTime);
		debugHistCount++;
		wsprintfA(tempStr, "%d", tyrian2_ftoi(1000.0f / (debugHist / debugHistCount)));
		wsprintfA(buffer, "X:%d Y:%-5d  %s FPS  %d %d %d %d", (mapX - 1) * 12 + player[0].x, curLoc, tempStr, player[0].x_velocity, player[0].y_velocity, player[0].x, player[0].y);
	...
	if (levelTimer && levelTimerCountdown > 0)
	{
		...
		wsprintfA(buffer, "%d.%d", levelTimerCountdown / 100, (levelTimerCountdown % 100) / 10);
	...
	if (isNetworkGame)
	{
		if (!reallyEndLevel)
		{
			...
			{
				if (!(SDLNet_Read16(&packet_state_in[0]->data[26]) == SDLNet_Read16(&packet_state_out[network_delay]->data[26]))) for (;;) Sleep(1000);
				...
				for (int i = 0; i < 2; i++)
				{
					...
					{
						char temp[64];
						wsprintfA(temp, "Player %d is unsynchronized!", i + 1);
...
void JE_loadMap(void)
{
	...
	JE_integer yy;
	/*
	 * Xbox/XDK builds can have a much smaller default stack than desktop builds.
	 * These buffers are large enough to overflow the stack when JE_loadMap() is
	 * entered from either Start Game or demo playback, causing a hard bugcheck
	 * before gameplay has a chance to draw. Keep them static so the level loader
	 * does not consume ~75 KB of stack in one frame.
	 */
...
new_game:
			...
			do
			{
				...
				if (s[0] == ']')
				{
					...
					case 'Q':
						...
						{
							for (uint i = 0; i < 2; ++i)
								wsprintfA(levelWarningText[i], "%s %lu", miscText[40], player[i].cash);
							strcpy(levelWarningText[2], "");
							levelWarningLines = 3;
						}
						else
						{
							wsprintfA(levelWarningText[0], "%s %lu", miscText[37], JE_totalScore(&player[0]));
						...
						if (jumpBackToEpisode1 && !twoPlayerMode)
						{
							...
							if (superArcadeMode < SA_ENGAGE)
							{
								...
								{
									wsprintfA(buffer, "%s %s", miscTextB[4], pName[0]);
									JE_dString(VGAScreen, JE_fontCenter(buffer, FONT_SHAPES), 100, buffer, FONT_SHAPES);

									wsprintfA(buffer, "Or play... %s", specialName[7]);
								...
								wsprintfA(buffer, "Type %s at Title", specialName[SANextShip[superArcadeMode] - 1]);
	...
	/* Read Shapes.DAT */
	wsprintfA(tempStr, "shapes%c.dat", tolower((unsigned char)char_shapeFile));
...
{
	// DIAGNOSTIC
	Sleep(500); // timing preserved from removed diag marker
	...
	Sleep(500); // timing preserved from removed diag marker // GREEN = entering loadCompShapes check
	if (shopSpriteSheet.data == NULL)
		JE_loadCompShapes(&shopSpriteSheet, '1');  // need mouse pointer sprites
	Sleep(500); // timing preserved from removed diag marker // BLUE = past loadCompShapes
	...
	for (; ; )
	{
		if (restart)
		{
			Sleep(500); // timing preserved from removed diag marker // ORANGE: restart block entered
			play_song(SONG_TITLE);
			Sleep(500); // timing preserved from removed diag marker // YELLOW: past play_song
			JE_loadPic(VGAScreen, 4, false);
			Sleep(500); // timing preserved from removed diag marker // CYAN: past JE_loadPic
			...
			if (moveTyrianLogoUp)
			{
				...
				Sleep(300); // timing preserved from removed diag marker // MAGENTA: logoUp branch
			...
			{
				Sleep(300); // timing preserved from removed diag marker // RED: before blit_sprite
				blit_sprite(VGAScreenSeg, 11, 4, PLANET_SHAPES, 146); // tyrian logo
				Sleep(300); // timing preserved from removed diag marker // GREEN: before fade_palette
				fade_palette(colors, 10, 0, 255 - 16);
				Sleep(300); // timing preserved from removed diag marker // BLUE: after first fade_palette
			}
			...
			Sleep(300); // timing preserved from removed diag marker // ORANGE: before second fade_palette
			// Fade in menu items.
			fade_palette(colors, 20, 255 - 16 + 1, 255);
			Sleep(300); // timing preserved from removed diag marker // WHITE: restart block done
...
bool newGame(void)
{
	if (gameplaySelect())
	{
		...
		else if (gameLoaded)
		{
			...
			if (!(episodeNum >= 1 && episodeNum <= EPISODE_AVAILABLE)) for (;;) Sleep(1000);
...
void newSuperTyrianGame(void)
{
	...
	char buf[10 + 1 + 15 + 1];
	wsprintfA(buf, "%s %s", miscTextB[4], pName[0]);
...
uint JE_makeEnemy(struct JE_SingleEnemyType* enemy, Uint16 eDatI, Sint16 uniqueShapeTableI)
{
	...
	if (sprite2s != NULL)
		...
		do { char _m[80]; wsprintfA(_m, "warning: ignoring sprite from unloaded shape table %d\n", shapeTableI); OutputDebugStringA(_m); } while (0);
	...
	if (enemyDat[eDatI].value > 1 && enemyDat[eDatI].value < 10000)
	{
		...
		case DIFFICULTY_WIMP:
			tempValue = tyrian2_ftoi(enemyDat[eDatI].value * 0.75f);
			break;
		...
		case DIFFICULTY_HARD:
			tempValue = tyrian2_ftoi(enemyDat[eDatI].value * 1.125f);
			break;
		case DIFFICULTY_IMPOSSIBLE:
			tempValue = tyrian2_ftoi(enemyDat[eDatI].value * 1.5f);
			break;
		case DIFFICULTY_INSANITY:
			tempValue = enemyDat[eDatI].value * 2;
			break;
		case DIFFICULTY_SUICIDE:
			tempValue = tyrian2_ftoi(enemyDat[eDatI].value * 2.5f);
			break;
	...
	if (enemyDat[eDatI].armor > 0)
	{
		if (enemyDat[eDatI].armor != 255)
		{
			...
			case -1:
			case DIFFICULTY_WIMP:
				tempArmor = tyrian2_ftoi(enemyDat[eDatI].armor * 0.5f + 1);
				break;
			case DIFFICULTY_EASY:
				tempArmor = tyrian2_ftoi(enemyDat[eDatI].armor * 0.75f + 1);
				break;
			case DIFFICULTY_NORMAL:
				tempArmor = enemyDat[eDatI].armor;
				break;
			case DIFFICULTY_HARD:
				tempArmor = tyrian2_ftoi(enemyDat[eDatI].armor * 1.2f);
				break;
			case DIFFICULTY_IMPOSSIBLE:
				tempArmor = tyrian2_ftoi(enemyDat[eDatI].armor * 1.5f);
				break;
			case DIFFICULTY_INSANITY:
				tempArmor = tyrian2_ftoi(enemyDat[eDatI].armor * 1.8f);
				break;
			case DIFFICULTY_SUICIDE:
...
void JE_eventSystem(void)
{
	...
	case 5:  // load enemy shape banks
	{
		...
		for (unsigned int i = 0; i < COUNTOF(newEnemyShapeTables); ++i)
		{
			if (enemySpriteSheetIds[i] != newEnemyShapeTables[i])
			{
				...
				{
					if (!(newEnemyShapeTables[i] <= COUNTOF(shapeFile))) for (;;) Sleep(1000);
	...
	case 16:
		...
		{
			OutputDebugStringA("warning: event 16: bad event data\n");
		}
		else
		{
	...
	case 25: /* Enemy Global Damage change */
		for (temp = 0; temp < 100; temp++)
		{
			...
			{
				if (galagaMode)
					enemy[temp].armorleft = tyrian2_ftoi(eventRec[eventLoc - 1].eventdat * (difficultyLevel / 2));
				else
	...
	default:
		do { char _m[80]; wsprintfA(_m, "warning: ignoring unknown event %d\n", eventRec[eventLoc - 1].eventtype); OutputDebugStringA(_m); } while (0);
		break;
	}
...
void JE_whoa(void)
{
	...
	if (!((unsigned)VGAScreen2->h * VGAScreen2->pitch >= screenSize &&
		(unsigned)game_screen->h * game_screen->pitch >= screenSize)) for (;;) Sleep(1000);
