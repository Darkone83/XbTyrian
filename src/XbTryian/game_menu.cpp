...
static __inline int gmenu_ftoi(float x)
{
	int result;
	__asm fld x
	__asm fistp result
...
static Uint8* playeritem_map(PlayerItems* items, uint i)
{
	...
	if (!(i < COUNTOF(map))) for (;;) Sleep(1000);
	return map[i];
}
	...
	do
	{
			...
			for (int x = min; x <= max; x++)
			{
				...
				if (x < max) /* x == max isn't a save slot */
				{
					...
					else
					{
						...
						wsprintfA(buf, "%s%d", miscTextB[1 - 1], saveFiles[x - 2].episode);
		...
		if (curMenu == MENU_JOYSTICK_CONFIG)
		{
			...
			for (uint i = 0; i < COUNTOF(menu_item); i++)
			{
				...
				{
					wsprintfA(value, "%d", joystick_config + 1);
				}
				else if (i == 1) // joystick is analog
				{
					wsprintfA(value, "%s", joystick[joystick_config].analog ? "TRUE" : "FALSE");
				}
				else if (i < 4)  // joystick analog settings
				{
					if (!joystick[joystick_config].analog)
						temp -= 3;
					wsprintfA(value, "%d", i == 2 ? joystick[joystick_config].sensitivity : joystick[joystick_config].threshold);
				}
		...
		if (curMenu == MENU_UPGRADE_SUB)
		{
			...
			for (tempW = 1; tempW < menuChoices[curMenu]; tempW++)
			{
				...
				case 1: /* ship */
					if (temp > 90)
						wsprintfA(tempStr, "Custom Ship %d", temp - 90);
					else
				...
				if (tempW != menuChoices[curMenu] - 1)
				{
					...
					wsprintfA(buf, "Cost: %d", temp_cost);
				...
				(curSel[MENU_UPGRADES] >= 1 && curSel[MENU_UPGRADES] <= 6)))
		{
			if (curMenu != MENU_UPGRADE_SUB)
			{
				...
				wsprintfA(buf, "%lu", player[0].cash);
				...
				(curSel[MENU_UPGRADES] == 2 || curSel[MENU_UPGRADES] == 5)))
		{
			if (twoPlayerMode)
			{
				...
				for (uint i = 0; i < 2; ++i)
				{
					wsprintfA(buf, "%s %lu", miscText[40 + i], player[i].cash);
		...
		if (curMenu == MENU_2_PLAYER_ARCADE)
		{
			for (uint i = 0; i < COUNTOF(inputDevice); i++)
			{
				...
				if (joysticks > 1 && inputDevice[i] > 2)
					wsprintfA(temp, "%s %d", inputDevices[2], inputDevice[i] - 2);
				else
					wsprintfA(temp, "%s", inputDevices[inputDevice[i] - 1]);
		...
		else
		{
			do
			{
				...
				if (curMenu == MENU_DATA_CUBE_SUB)
				{
					...
					char buf[55];
					wsprintfA(buf, "%s %d%%", miscText[11], percent_read);
...
void draw_ship_illustration(void)
{
	...
	// ship
	{
		if (!(player[0].items.ship > 0)) for (;;) Sleep(1000);
	...
	// generator
	{
		if (!(player[0].items.generator > 0 && player[0].items.generator < 7)) for (;;) Sleep(1000);
...
void JE_updateNavScreen(void)
{
	...
	tempNavX = gmenu_ftoi(navX);
	tempNavY = gmenu_ftoi(navY);
...
void JE_scaleBitmap(SDL_Surface* dst_bitmap, const SDL_Surface* src_bitmap, int x1, int y1, int x2, int y2)
{
	...
	if (!(src_bitmap != NULL && dst_bitmap != NULL)) for (;;) Sleep(1000);
	if (!(x1 >= 0 && y1 >= 0 && x2 < src_bitmap->pitch && y2 < src_bitmap->h)) for (;;) Sleep(1000);
	...
	for (int i = 0; i < h; i++)
	{
		...
		src = src_w = (Uint8*)(src_bitmap->pixels) + (src_bitmap->w * ((unsigned int)gmenu_ftoi(cumulative_skip_h)));
		...
		for (int j = 0; j < w; j++)
		{
			...
			*dst = *src;
			...
			src = src_w + ((unsigned int)gmenu_ftoi(cumulative_skip_w)); //value is floored
		}
...
void JE_computeDots(void)
{
	...
	for (x = 0; x < mapPNum; x++)
	{
		...
		if (tempX != 0)
			planetDots[x] = gmenu_ftoi(sqrtf(sqrtf((distX * distX) + (distY * distY)))) - 1;
		else
			planetDots[x] = 0;
...
void JE_scaleInPicture(SDL_Surface* dst, const SDL_Surface* src)
{
	for (int i = 2; i <= 160; i += 2)
	{
		...
		JE_scaleBitmap(dst, src, 160 - i, 0, 160 + i - 1, 100 + gmenu_ftoi(i * 0.625f) - 1);
		JE_showVGA();
